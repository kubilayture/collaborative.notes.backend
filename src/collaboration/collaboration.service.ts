...
export class CollaborationService implements OnModuleInit {
    ...
    this.hocusPocusServer = new HocusPocusServer({
      ...
      onLoadDocument: async (data) => {
        ...
        try {
          ...
          this.logger.log(`🔍 Note found: ${!!note}, has content: ${!!(note?.content)}`);
          if (note?.content) {
            this.logger.log(`📄 Raw content type: ${typeof note.content}, value: ${JSON.stringify(note.content).substring(0, 200)}...`);
          }
          ...
          if (note && note.content) {
            ...
            if (typeof note.content === 'string') {
              ...
              this.logger.log(`📝 Using string content: ${contentText.substring(0, 100)}...`);
              ...
              if (note.content.data && typeof note.content.data === 'string') {
                ...
                this.logger.log(`📝 Using object.data content: ${contentText.substring(0, 100)}...`);
              } else if (note.content.text) {
                contentText = note.content.text;
                this.logger.log(`📝 Using object.text content: ${contentText.substring(0, 100)}...`);
                ...
                this.logger.log(`📝 Using converted TipTap content: ${contentText.substring(0, 100)}...`);
              } else {
                // Fallback: stringify the object
                contentText = JSON.stringify(note.content);
                this.logger.log(`📝 Using stringified content: ${contentText.substring(0, 100)}...`);
              }
            }
            
            if (contentText) {
              ytext.insert(0, contentText);
              this.logger.log(`✅ Loaded document ${noteId} with content length: ${contentText.length}`);
              return Y.encodeStateAsUpdate(ydoc);
            }
          }
          
          this.logger.log(`❌ Document ${noteId} not found or empty, starting with empty document`);
          return null; // Return null to start with empty document
        } catch (error) {
          this.logger.warn(`⚠️ Failed to load document ${noteId}:`, error.message);
          return null;
        }
      },
      ...
      onStoreDocument: async (data) => {
        ...
        try {
          ...
          this.logger.log(`📝 Document ${noteId} content updated: ${textContent.substring(0, 100)}...`);
          ...
          this.logger.error(`❌ Failed to process document ${noteId}:`, error.message);
        }
      },

      onConnect: async (data) => {
        this.logger.log(`🔌 Client connected to document: ${data.documentName}, socketId: ${data.socketId}`);
      },

      onDisconnect: async (data) => {
        this.logger.log(`🔌 Client disconnected from document: ${data.documentName}, socketId: ${data.socketId}`);
      },
    });
  ...
  private async startServer() {
    try {
      ...
      this.logger.log('🚀 HocusPocus collaboration server started on port 8080');
    } catch (error) {
