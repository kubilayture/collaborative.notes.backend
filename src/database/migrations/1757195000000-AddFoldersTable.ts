...
export class AddFoldersTable1757195000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Create indexes
    await queryRunner.query(
      `CREATE INDEX "IDX_folders_owner_id" ON "folders" ("ownerId") `,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_folders_parent_id" ON "folders" ("parentId") `,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_folders_owner_parent" ON "folders" ("ownerId", "parentId") `,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_notes_folder_id" ON "notes" ("folderId") `,
    );

    // Add foreign key constraints
    await queryRunner.query(
      `ALTER TABLE "folders" ADD CONSTRAINT "FK_folders_owner" FOREIGN KEY ("ownerId") REFERENCES "user"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "folders" ADD CONSTRAINT "FK_folders_parent" FOREIGN KEY ("parentId") REFERENCES "folders"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "notes" ADD CONSTRAINT "FK_notes_folder" FOREIGN KEY ("folderId") REFERENCES "folders"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "notes" DROP CONSTRAINT "FK_notes_folder"`,
    );
    await queryRunner.query(
      `ALTER TABLE "folders" DROP CONSTRAINT "FK_folders_parent"`,
    );
    await queryRunner.query(
      `ALTER TABLE "folders" DROP CONSTRAINT "FK_folders_owner"`,
    );
